...
export const videosValidations = {
  ...
  availableResolutions: body('availableResolutions').optional({ nullable: true }).isArray({ min: 1 }).custom(isResolutionType).withMessage('Resolutions should not be empty'),
...
function isResolutionType(resolutionsTypes: ResolutionsTypes[] | any[]) {
  return resolutionsTypes.every((resolutionsType) => {
    return Object.values(ResolutionsTypes).includes(resolutionsType)
  })
}
